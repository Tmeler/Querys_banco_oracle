 SELECT M.ORG_TAB_IN_CODIGO||'|'||M.ORG_PAD_IN_CODIGO||'|'||M.ORG_IN_CODIGO||'|'||M.ORG_TAU_ST_CODIGO||'|'||M.MOV_TAB_IN_CODIGO||'|'||M.MOV_SEQ_IN_CODIGO||'|'||M.MOV_IN_NUMLANCTO AS ID_MOVFIN,
       ...
       LPR.LCL_CH_NATUREZA AS NATUREZA

  FROM CONSTRUCAO.FIN_MOVIMENTO@HACASA M

LEFT JOIN CONSTRUCAO.FIN_REFERENCIAFIN@HACASA R ON
  (R.REF_ORG_TAB_IN_CODIGO = M.ORG_TAB_IN_CODIGO AND
   R.REF_ORG_PAD_IN_CODIGO = M.ORG_PAD_IN_CODIGO AND
   R.REF_ORG_IN_CODIGO     = M.ORG_IN_CODIGO AND
   R.REF_ORG_TAU_ST_CODIGO = M.ORG_TAU_ST_CODIGO AND
   R.REF_MOV_TAB_IN_CODIGO = M.MOV_TAB_IN_CODIGO AND
   R.REF_MOV_SEQ_IN_CODIGO = M.MOV_SEQ_IN_CODIGO AND
   R.REF_MOV_IN_NUMLANCTO  = M.MOV_IN_NUMLANCTO)

LEFT JOIN CONSTRUCAO.FIN_LANCCLASSE@HACASA LCL ON
  (LCL.ORG_TAB_IN_CODIGO = R.ORG_TAB_IN_CODIGO AND
   LCL.ORG_PAD_IN_CODIGO = R.ORG_PAD_IN_CODIGO AND
   LCL.ORG_IN_CODIGO     = R.ORG_IN_CODIGO AND
   LCL.ORG_TAU_ST_CODIGO = R.ORG_TAU_ST_CODIGO AND
   LCL.MOV_TAB_IN_CODIGO = R.MOV_TAB_IN_CODIGO AND
   LCL.MOV_SEQ_IN_CODIGO = R.MOV_SEQ_IN_CODIGO AND
   LCL.MOV_IN_NUMLANCTO  = R.MOV_IN_NUMLANCTO)
